...
subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', 'trl>=0.15.0', 'transformers', 'accelerate', 'peft', 'datasets', 'bitsandbytes', 'scipy'], check=True)
try:
    subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', '--no-deps', 'unsloth'], check=True)
except Exception:
    pass  # unsloth optional
...
WORK='/kaggle/working'; REPO=f'{WORK}/edupath'
